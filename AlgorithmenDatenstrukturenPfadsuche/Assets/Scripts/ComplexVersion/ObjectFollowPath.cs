...
namespace ComplexVersion
{
    public class ObjectFollowPath : MonoBehaviour
    {
        ...
        */
        ...
        void Update()
        {
            ...
            if (moving)
            {
                ...
                if (!reachedGoal)
                {
                    ...
                    ChangeOrientation(g.path[index].GetGlobalPosition());
        ...
        void ChangeOrientation(Vector3 newPos)
        {
            //Vector3 direction = this.transform.position - newPos;
            Quaternion rotation = Quaternion.LookRotation(newPos);
            transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, speed * Time.deltaTime * 10);
        }
    }
}